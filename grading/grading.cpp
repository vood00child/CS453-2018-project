...
class TransactionalLibrary final : private NonCopyable
{
  ...
  private:
    ...
    **/
    ...
    auto solve(char const *name) const
    {
    ...
    template <class Signature>
    void solve(char const *name, Signature &func) const
    {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (!segment_next)
                    {
                        ...
                        { // Allocate
                            if (segment_count < nbaccounts)
                            { // Just "allocate" account
  ...
  public:
    ...
    **/
    void worker_notify(bool success) noexcept
    {
        ...
        if (res + 1 == nbworkers)
        { // Latest worker, switch to done/fail status
            nbready.store(0, ::std::memory_order_relaxed);
    ...
    { // Start threads
        threads[i] = ::std::thread{[&](unsigned int i) {
                                       ...
                                       catch (::std::exception const &err)
                                       {
                                           ...
                                           ::std::cerr << "⎧ *** EXCEPTION - worker thread ***" << ::std::endl
                                                       << "⎩ " << err.what() << ::std::endl;
                                           return;
                                       }
                                   },
...
int main(int argc, char **argv)
{
    try
    {
        ...
        // auto const slow_factor = 2ul; // TO be changed
        auto const slow_factor = 1000ul; // TO be changed
        ...
        auto &&eval = [&](char const *path, Chrono::Tick reference) { // Library evaluation
            try
            {
                ...
                catch (Exception::TooSlow const &err)
                { // Special case since interrupting threads may lead to corrupted state
